...
if (isset($_GET['delete_id'])) {
    ...
    $deleteQuery = "DELETE FROM agent WHERE userid='$delete_id'";
    
    if (mysqli_query($conn, $deleteQuery)) {
        echo "<script>alert('User deleted successfully!'); 
        window.location.href='agent_list.php'</script>";
        exit();
    } else {
...
$countQuery = "SELECT COUNT(*) AS total FROM agent WHERE  (fullname LIKE '%$search%' OR email LIKE '%$search%' OR mobile LIKE '%$search%')";
$countResult = mysqli_query($conn, $countQuery);
$totalRows = mysqli_fetch_assoc($countResult)['total'];
$totalPages = ceil($totalRows / $limit);

// Fetch users with pagination & search
$query = "SELECT a.*,r.title,r.addr,r.city,r.pincode FROM agent a left join restaurant r on  resid=res_id  WHERE  (a.fullname LIKE '%$search%' OR a.email LIKE '%$search%' OR a.mobile LIKE '%$search%' ) LIMIT $offset, $limit";
            ...
            <tbody id="userTable">
                <?php while ($row = mysqli_fetch_assoc($result)): ?>
                    <tr>
                        ...
                        <small><?php  echo $row['addr']." ".$row["city"]."-".$row['pincode']?></small>
                    </td>
                        <td>
                        ...
                        <td>
                            ...
                            <a href="?delete_id=<?php echo $row['userid']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure?')">Delete</a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
