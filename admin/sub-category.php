...
if (isset($_POST['submit'])) {
    ...
    $target_file =   time() . "_" . $subcategoryname; // Unique file name
    move_uploaded_file($file["tmp_name"], $target_dir.$target_file);

    // Insert into DB
    $sql = "INSERT INTO `subcategory`(subcategoryname, categoryid, subcategoryimg) VALUES ('$name', '$category_id', '$target_file')";
    
    if (mysqli_query($conn, $sql)) {
       
        $success_message = "Subcategory added successfully!";
    } else {
        $error_message = "Error! Subcategory not added.";
    }
}
...
if (isset($_GET['delete_id'])) {
    ...
    $fetchQuery = "SELECT subcategoryimg FROM subcategory WHERE subcategoryid = $id";
    $fetchResult = mysqli_query($conn, $fetchQuery);
    $row = mysqli_fetch_assoc($fetchResult);
    
    if ($row) {
        $imagePath = $row['subcategoryimg']; 
        ...
        if (mysqli_query($conn, $sql)) {
            echo "<script>alert('Subcategory deleted successfully!'); window.location='sub-category.php';</script>";
        }
    }
}
...
if (isset($_GET['ajax'])) {
    ...
    $countQuery = "SELECT COUNT(*) as total FROM subcategory WHERE subcategoryname LIKE '%$search%' ";
    $countResult = mysqli_query($conn, $countQuery);
    $totalRecords = mysqli_fetch_assoc($countResult)['total'];
    $totalPages = ceil($totalRecords / $limit);

    // Fetch Data
    $sql = "SELECT s.*, c.categoryname FROM subcategory s 
            JOIN category c ON s.categoryid = c.categoryid 
            WHERE s.subcategoryname LIKE '%$search%' or c.categoryname LIKE '%$search%' 
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<td><img src='uploads/" . $row['subcategoryimg'] . "' width='50' height='50'></td>";
        echo "<td>

<a href='?delete_id=" . $row['subcategoryid'] . "' class='btn btn-danger btn-sm ' onclick='return confirm(\"Are you sure?\")'>
                          <i class='fa fa-trash'></i>  Delete
                    </a>
                       <a href='edit-subcategory.php?edit_id=" . $row['subcategoryid'] . "' class='btn btn-success  btn-sm' >
...
<div class="content bg-light" id="mainContent">
    <form method="post" enctype="multipart/form-data">
        <div class="row ms-3 mt-2">
            <div class="col-md-4 shadow bg-white border-0 rounded p-3 me-5" style="max-height:500px">
                ...
                <h3 class="p-2 fw-bold">Add Subcategory</h3>
                <hr>
                ...
                <!-- Subcategory Name -->
                <label for="subcategoryname" class="label-control mb-1 fw-bold">Subcategory Name</label>
                <!-- <input type="text" name="subcategoryname" class="form-control mb-3" placeholder="Enter Subcategory Name" required> -->
    ...
    class="form-control mb-3"
       ...
       title="Sub Category Name must be 3-30 characters long and contain only letters and spaces.">
                ...
                <!-- subcategoryimage Upload -->
                <label for="img" class="label-control mb-1 fw-bold">Subcategory subcategoryimage</label>
                ...
                <table class="table table-bordered">
                    <thead class=" ">
                        <tr>
                            ...
                            <th class='bg-warning text-white'>Subcategory</th>
